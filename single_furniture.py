...
def feature_extraction_InV3(img_width, img_height,
    ...
    target_size = (299, 299),
    batch_size = 15, #每次進來訓練並更新學習率的圖片數 -> memory leak -> 調低此參數
    class_mode = "categorical",
    shuffle=False)

    y_train=train_generator.classes
    y_train1 = np.zeros((num_image, 2)) # 2 , 依據class數量而定, np.zeros -> 宣告全部為0的空陣列
    #2009,2
    y_train1[np.arange(num_image), y_train] = 1 #np.arrange
...
def train_last_layer(img_width, img_height,
    ...
    my_model.save('./test/inv3_single_furniture_binary.h5')
    return history
...
if __name__=="__main__":
    ...
    test_data_dir = "./Test"
    num_image=2609 #Train的圖片總數
    num_test_image = 400 #Test的圖片總數
    epochs = 10 #回合的概念
